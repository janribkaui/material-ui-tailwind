...
import { getLineFeed } from '@janribkaui/internal-docs-utils';
...
export function getSystemComponents() {
  if (!systemComponents) {
    systemComponents = fs
      .readdirSync(path.resolve('packages', 'janribkaui-system', 'src'))
...
export function getJrName(name: string) {
  return `Jr${name.replace('Styled', '')}`;
}
...
export function extractPackageFile(filePath: string) {
  ...
  return {
    ...result,
    jrPackage: result.packagePath?.replace('x-', 'janribkaui-'),
  };
}
